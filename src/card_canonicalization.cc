...
constexpr std::array<SuitPermutation, 24> BuildSuitPermutations() {
  ...
  SuitPermutation current = {Suit::Hearts, Suit::Diamonds, Suit::Clubs, Suit::Spades};
  size_t index = 0;
  do {
...
uint64_t EncodeBoard(const Board& board, const SuitPermutation& permutation) noexcept {
  const auto cards = board.cards();
  std::array<Card, kMaxBoardCards> mapped = {};
  for (size_t index = 0; index < cards.size(); ++index) {
    mapped[index] = PermuteCard(cards[index], permutation);
  }
  std::ranges::sort(std::span(mapped).first(std::min<size_t>(3, cards.size())));
...
ComboId PermuteCombo(ComboId hand, const SuitPermutation& permutation) noexcept {
  const auto cards = hand.cards();
  return CardsToComboId(PermuteCard(cards[0], permutation), PermuteCard(cards[1], permutation));
}
...
PrivateObservationId CanonicalPrivateObservation(ComboId hand, const Board& board) noexcept {
  std::pair<uint64_t, uint16_t> best{std::numeric_limits<uint64_t>::max(),
