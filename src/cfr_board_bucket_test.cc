...
TEST_CASE("coarse private histories use growing storage") {
  SolverConfig config;
  ...
  CHECK_FALSE(first.public_state_prebuild_complete);
  CHECK_FALSE(first.info_set_prebuild_complete);
  CHECK_FALSE(first.frozen_info_set_lookup_prebuild_complete);
  CHECK(first.warmup_iterations == 4);
  CHECK(first.frozen_iterations == 0);
  ...
  CHECK(second.warmup_iterations == 2);
  CHECK(second.frozen_iterations == 0);
}
