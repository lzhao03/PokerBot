...
inline const auto kStraightDensity = [] {
  std::array<uint8_t, 8192> table = {};
  for (size_t mask = 0; mask < table.size(); ++mask) {
    int density = 0;
    for (int start = 0; start <= 8; ++start) {
      density = std::max(
          density, std::popcount((mask >> start) & size_t{0x1F}));
    }
    constexpr size_t wheel_mask = (size_t{1} << 12) | 0x0F;
    density = std::max(density, std::popcount(mask & wheel_mask));
    table[mask] = static_cast<uint8_t>(density);
  }
  return table;
}();
...
absl::Status ValidateEquityModelShape(const EquityBucketModel& model) {
  ...
  for (StreetKind street : {StreetKind::Preflop, StreetKind::River}) {
...
PrivateBucketId EquityBucket(StreetKind street,
  ...
  assert(street == StreetKind::Flop || street == StreetKind::Turn);
  const size_t index = static_cast<size_t>(street);
...
PrivateObservationId EquityObservation(
  ...
  if (abstraction.config().recall_mode == RecallMode::CurrentBucketOnly) {
    return PrivateObservationId(static_cast<uint64_t>(current) + 1);
  }
...
absl::StatusOr<CardAbstraction> CardAbstraction::Create(
  ...
  if (config.private_kind == PrivateAbstractionKind::EquityPotential) {
...
PrivateObservationId ObservePrivate(const CardAbstraction& abstraction,
  ...
  switch (abstraction.config().private_kind) {
    ...
    case PrivateAbstractionKind::EquityPotential:
