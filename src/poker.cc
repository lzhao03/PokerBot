...
int HighestStraight(uint16_t rank_mask) noexcept {
  ...
  if (starts != 0) return std::bit_width(starts) + 3;
...
constexpr uint32_t PackHand(HandCategory category, uint16_t primary, uint16_t kickers = 0) noexcept {
  return (std::to_underlying(category) << 26) | (static_cast<uint32_t>(primary) << 13) | kickers;
}
...
uint32_t EvaluateSeven(const HandFeatures& features) noexcept {
  ...
  for (uint16_t suit_mask : features.suit_masks) {
  ...
  if (features.quad_mask != 0) {
    ...
    return PackHand(HandCategory::FourOfAKind, static_cast<uint16_t>(four), static_cast<uint16_t>(kicker));
  }

  if (features.trip_mask != 0 && std::popcount(features.pair_mask) > 1) {
    const int three = std::bit_width(features.trip_mask) - 1;
    const int pair = std::bit_width(static_cast<uint16_t>(features.pair_mask & ~(1U << three))) - 1;
    return PackHand(HandCategory::FullHouse, static_cast<uint16_t>(three), static_cast<uint16_t>(pair));
  }

  if (flush_mask != 0) return PackHand(HandCategory::Flush, HighestRanks(flush_mask, 5));
  ...
  if (features.trip_mask != 0) {
    const int three = std::bit_width(features.trip_mask) - 1;
    const uint16_t remaining = static_cast<uint16_t>(rank_mask & ~(1U << three));
    return PackHand(HandCategory::ThreeOfAKind, static_cast<uint16_t>(three), HighestRanks(remaining, 2));
  }
  if (std::popcount(features.pair_mask) >= 2) {
    const uint16_t pairs = HighestRanks(features.pair_mask, 2);
    const uint16_t remaining = static_cast<uint16_t>(rank_mask & ~pairs);
    return PackHand(HandCategory::TwoPair, pairs, HighestRanks(remaining, 1));
  }
  if (features.pair_mask != 0) {
    const int pair = std::bit_width(features.pair_mask) - 1;
    const uint16_t remaining = static_cast<uint16_t>(rank_mask & ~(1U << pair));
    return PackHand(HandCategory::Pair, static_cast<uint16_t>(pair), HighestRanks(remaining, 3));
  }
